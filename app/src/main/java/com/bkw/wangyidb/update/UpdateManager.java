package com.bkw.wangyidb.update;
...
import static com.bkw.wangyidb.db.BaseDaoFactory.PACKAGENAME;
...
public class UpdateManager {
    ...
    private SQLiteDatabase getDb(Integer id) {
        ...
        File file = new File("data/data/" + PACKAGENAME + "u_" + id + "_private.db");
        if (!file.exists()) {
            ...
            if (step.getVersionFrom() == null || step.getVersionTo() == null) {
                ...
                if (versionArray != null && versionArray.length > 0) {
                    for (int i = 0; i < versionArray.length; i++) {
                        // APP数据库当前版本号数据保存在sp里面 或者文本文件，这里V002 就代表当前版本信息
                        // V003 应该从服务器获取
